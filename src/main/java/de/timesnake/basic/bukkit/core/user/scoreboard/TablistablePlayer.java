...
public interface TablistablePlayer extends
    de.timesnake.basic.bukkit.util.user.scoreboard.TablistablePlayer {
  ...
  default String getFullPrefix(LinkedList<TablistGroupType> types) {
    ...
    for (TablistGroupType type : types) {
      ...
      if (!(group == null || group.getTablistPrefix() == null
          || group.getTablistPrefixChatColor() == null)) {
        prefixBuilder.append(group.getTablistPrefixChatColor()).append(group.getTablistPrefix())
            .append("§r");
      }
    }
