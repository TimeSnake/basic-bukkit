...
public class InventoryDelegation implements Inventory {
  ...
  /**
   * Stores the given ItemStacks in the inventory. This will try to fill existing stacks and empty
   * slots as well as it can.
   * <p>
   * The returned HashMap contains what it couldn't store, where the key is the index of the
   * parameter, and the value is the ItemStack at that index of the varargs parameter. If all items
   * are stored, it will return an empty HashMap.
   * <p>
   * If you pass in ItemStacks which exceed the maximum stack size for the Material, first they will
   * be added to partial stacks where Material.getMaxStackSize() is not exceeded, up to
   * Material.getMaxStackSize(). When there are no partial stacks left stacks will be split on
   * Inventory.getMaxStackSize() allowing you to exceed the maximum stack size for that material.
   * <p>
   * It is known that in some implementations this method will also set the inputted argument amount
   * to the number of that item not placed in slots.
   *
   ...
   * <p>
   * The returned HashMap contains what it couldn't remove, where the key is the index of the
   * parameter, and the value is the ItemStack at that index of the varargs parameter. If all the
   * given ItemStacks are removed, it will return an empty HashMap.
   * <p>
   * It is known that in some implementations this method will also set the inputted argument amount
   ...
   * <p>
   * Similar to {@link Inventory#removeItem(ItemStack...)} in behavior, except this method will
   * check all possible slots in the inventory, rather than just the main storage contents.
   * <p>
   * It will try to remove 'as much as possible' from the types and amounts you give as arguments.
   * <p>
   * The returned HashMap contains what it couldn't remove, where the key is the index of the
  ...
  public @NotNull HashMap<Integer, ItemStack> removeItemAnySlot(@NotNull ItemStack... items)
      throws IllegalArgumentException {
    return getInventory().removeItemAnySlot(items);
  }
  ...
  /**
   * Completely replaces the inventory's contents. Removes all existing contents and replaces it
   * with the ItemStacks given in the array.
   *
   * @param items A complete replacement for the contents; the length must be less than or equal to
   *              {@link #getSize()}.
  ...
  /**
   * Return the contents from the section of the inventory where items can reasonably be expected to
   * be stored. In most cases this will represent the entire inventory, but in some cases it may
   * exclude armor or result slots.
   * <br>
   * It is these contents which will be used for add / contains / remove methods which look for a
   * specific stack.
   *
  ...
  /**
   * Checks if the inventory contains any ItemStacks with the given material, adding to at least the
   * minimum amount specified.
   *
   * @param material The material to check for
   * @param amount   The minimum amount
   * @return true if amount is less than 1, true if enough ItemStacks were found to add to the given
   * amount
   * @throws IllegalArgumentException if material is null
   */
  @Override
  ...
  /**
   * Checks if the inventory contains at least the minimum amount specified of exactly matching
   * ItemStacks.
   * <p>
   ...
   * @return false if item is null, true if amount less than 1, true if amount of exactly matching
   * ItemStacks were found
  ...
  /**
   * Checks if the inventory contains ItemStacks matching the given ItemStack whose amounts sum to
   * at least the minimum amount specified.
   *
   * @param item   the ItemStack to match against
   * @param amount the minimum amount
   * @return false if item is null, true if amount less than 1, true if enough ItemStacks were found
   ...
   * in that slot. If no matching ItemStack with the given Material is found, an empty map is
   * returned.
   *
  ...
  /**
   * Finds all slots in the inventory containing any ItemStacks with the given ItemStack. This will
   * only match slots if both the type and the amount of the stack match
   * <p>
   * The HashMap contains entries where, the key is the slot index, and the value is the ItemStack
   * in that slot. If no matching ItemStack with the given Material is found, an empty map is
   * returned.
  ...
  /**
   * Returns the first slot in the inventory containing an ItemStack with the given stack. This will
  ...
  /**
   * Check whether or not this inventory is empty. An inventory is considered to be empty if there
   * are no ItemStacks in any slot of this inventory.
   *
  ...
  /**
   * Gets a list of players viewing the inventory. Note that a player is considered to be viewing
   * their own inventory and internal crafting screen even when said inventory is not open. They
   * will normally be considered to be viewing their inventory even when they have a different
   * inventory screen open, but it's possible for customized inventory screens to exclude the
  ...
  public @Nullable InventoryHolder getHolder(boolean useSnapshot) {
    return getInventory().getHolder(useSnapshot);
  }
  ...
  /**
   * Returns an iterator starting at the given index. If the index is positive, then the first call
   * to next() will return the item at that index; if it is negative, the first call to previous
   * will return the item at index (getSize() + index).
   *
